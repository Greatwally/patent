 ...
 * @subpackage Jtagmemberdirectory
 */
class JTagMemberDirectoryControllerCategories extends JControllerLegacy {
        ...
        switch($this->getTask())
		 {
             ...
             case 'cancel' :
				{
					JRequest::checkToken() or jexit('Invalid Token');
					$this->setRedirect( 'index.php?option=com_jtagmembersdirectory&c=categories' );;
                }
					break;
             case 'restore'     :
				{
                     ...
                     $post	= JRequest::get('post');
                      $model      = $this->getModel('categories');
                            ...
                            if ($model->store($post)) {
                                    ...
                                    $link = 'index.php?option=com_jtagmembersdirectory&c=categories';
                                    $msgType = 'message' ;
                            } else {
                                    $msg = $model->getError(); 
                                    $link = 'index.php?option=com_jtagmembersdirectory&c=categories&task=edit&cid[]='.$cid;
                                    $msgType = 'error' ;
                            }
                        ...
                        case 'edit'    :
			{
							...
							$row = $model->editCat(); 
							//JRequest::setVar( 'view'  , 'category');
							$view = $this->getView('category', 'html', 'jtagmemberdirectoryView');
							$view->assign('row', $row);
