...
class JtagMemberDirectoryModelusers extends JModelLegacy
{
    function addToMembers($cid){
...
//			echo $newuse[0]->name;
			$sql = "SELECT max(`id`) FROM `#__jtmb_members_directory`";
			$db->setQuery($sql);
			$cnt = $db->loadResult();
			$cnt=$cnt+1;
			
	$name= $newuse[0]->name; $email= $newuse[0]->email; $user_id = $newuse[0]->id;
			$sql = "INSERT INTO `#__jtmb_members_directory`(`id`,`user_id`,`profile_picture`,`name`,`Email`)
