...
class jtagobjectdirectoryModelobjects extends JModelLegacy
{
  /**
   * Items total
   * @var integer
...
//start changes added by sarika
  ...
  {//comented by sarika
   // $sql = 'SELECT id, user_id, name, phone_no, object_since, object_profile, profile_picture
//      FROM `#__jtmb_objects_directory` c ORDER BY name ASC';
...
// names sorted alphabetically 
		...
		//$query = ' SELECT *'
		//	. ' FROM #__jtmb_objects_directory as c INNER JOIN #__jtmb_assigned_categories AS b ';
           $query='SELECT DISTINCT c.id,c.user_id,c.profile_picture,c.name,c.Email,c.display_last_name,c.country,c.city,
c.state,c.phone_no,c.facebook_page,c.twitter_page,c.object_profile,c.display_in_frontend,c.allow_edit,c.hasGallery
FROM #__jtmb_objects_directory AS c';
...
//print_r($catwise);exit;
 ...
 if (isset($criteria['cust_id']) || isset($criteria['customtext']))
    {
    		$sql .= ' INNER JOIN #__jtmb_custom_maps_objects AS n';
   }
...
//print_r($userid);exit;
  ...
  $query1='SELECT ca.cat_id FROM #__jtmb_assigned_categories ca LEFT JOIN #__jtmb_objects_directory  cd ON ca.objectid=cd.id where cd.user_id='.$userid.' ';
...
//print_r($cat);exit;
 $sql  .= ' ,#__jtmb_assigned_categories AS b WHERE  b.cat_id ='.$cat.' AND c.id=b.objectid AND c.Published=1 ';
...
else
{
 $sql  .= ' WHERE c.display_in_frontend=1 AND deleted_at = 0 AND c.Published=1 ';
    ...
    if (isset($criteria['cat_id']))
    {
		 ...
		 $sql .= ' AND b.cat_id = "'.$criteria['cat_id'].'" AND c.id=b.objectid';
	}
        ...
        function retrive_custom()
{
   ...
   $query = 'SELECT distinct c.field_name, c.field_label, d.field_value, c.display_nr_cf,d.user_id FROM #__jtmb_custom_fields c LEFT JOIN #__jtmb_custom_maps_objects  d ON c.field_label=d.field_label ';
...
function saverequest($id,$name,$username,$email)
 {
  ...
  $db = & JFactory::getDBO();
    $sqluid = "SELECT * FROM #__jtmb_object_request where user_id= '".$id."';";
                          $db->setQuery($sqluid); 
  		          $rowsuserid = $db->loadObjectList();
           if(empty($rowsuserid))
{
				$query = "INSERT INTO #__jtmb_object_request (user_id,email,name,username) VALUES ('$id','$email','$name','$username')";
				$db->setQuery($query);
   $db->query();
}
 ...
 function  checkCustomField()
   ...
   {
       $db = & JFactory::getDBO();
        $query = "SELECT objectid,cat_id FROM #__jtmb_assigned_categories WHERE objectid=$mid";
       $db->setQuery($query);
       $mitems = $db->loadObjectList();
       return $mitems;
   }
