...
class jtagobjectdirectoryModeleditobjectdetail extends JModelLegacy {
  
	var $tmp_object;

	function getobjectDetail($id)
	{
		$query = 'SELECT * FROM #__jtmb_objects_directory WHERE id = '.$id.' LIMIT 1';
    	$this->_db->setQuery($query);
    	return $this->_db->loadObject();
	}

   function saveobjectDetail($data)
  {

    $query = 'SELECT id,field_label FROM #__jtmb_custom_fields';
     $this->_db->setQuery($query);
      $rows=$this->_db->loadAssocList();
     $delquery = 'DELETE FROM `#__jtmb_custom_maps_objects` WHERE user_id = ' . $data['user_id'];
...
if($k=="object_profile"){
    ...
    $val=$data[$i][value];
     $cfQuery = "INSERT INTO #__jtmb_custom_maps_objects (user_id, field_label,field_value)
                               VALUES ('$data[user_id]', '$label', '$val')" ;
       /* $cfQuery = "UPDATE #__jtmb_custom_maps_objects SET `field_value` = '".$data[$i]['value']."' WHERE `user_id` = ". $data['user_id'] ." AND `field_label`='".$rows[$i]['field_label']."'";*/
...
$catid=$data['cat_id'];
 ...
 $selquery = 'SELECT `id` FROM #__jtmb_objects_directory WHERE `user_id` = '.$id;
    $this->_db->setQuery($selquery);
    $pp = $this->_db->loadObject();
    $uid= $pp->id;

 $delquery = "DELETE FROM #__jtmb_assigned_categories WHERE `objectid` = ".$uid;
               $this->_db->setQuery($delquery);
                   $this->_db->query();  

foreach($catid as $cat)
{
 $catquery = "INSERT INTO #__jtmb_assigned_categories(`objectid`,`cat_id`) VALUES ('$uid','$cat') ";
    $this->_db->setQuery($catquery);
    $this->_db->query(); 

}
  
    $query = 'UPDATE #__jtmb_objects_directory SET '. implode(',',$upds) . ' WHERE user_id ='. $data['user_id'];
    $this->_db->setQuery($query);
    $this->_db->query();      
...
if($data['attachment'])       
    ...
    $query = 'UPDATE #__jtmb_objects_directory SET `hasGallery`=1 WHERE user_id ='. $data['user_id'];
    $this->_db->setQuery($query);
    $this->_db->query();   
  ...
  if(!empty($id))
{
 $query1 = 'SELECT user_id FROM `#__jtmb_objects_directory` where id='.$id;
        $this->_db->setQuery($query1);
        $rows =$this->_db->loadObject();
   $query = 'SELECT distinct c.field_name, c.field_label, d.field_value, c.display_nr_cf, c.allow_edit FROM #__jtmb_custom_fields c LEFT JOIN #__jtmb_custom_maps_objects  d ON d.user_id ='. $rows->user_id.' AND c.field_label=d.field_label ';
...
//print_r($this->_db->loadObjectList());
  ...
  /*function _uploadPicture($force_delete_old = false, $id) 
  {
    ...
    $query = 'SELECT `profile_picture` FROM #__jtmb_objects_directory WHERE `user_id` = '.$id;
    $this->_db->setQuery($query);
    $pp = $this->_db->loadObject();
    ...
    if (empty($file['name'])) {
		 $query = "UPDATE #__jtmb_objects_directory SET `profile_picture` = NULL WHERE `user_id` = ".$id;
    	   $this->_db->setQuery($query);
		   $this->_db->query();  
        return;
    }
    ...
    if (strtolower($ext) == 'jpg' || strtolower($ext) == 'png' || strtolower($ext) == 'gif'|| strtolower($ext) == 'jpeg') {
        if (JFile::upload($file['tmp_name'], $dest)) {
           ...
           $query = "UPDATE #__jtmb_objects_directory SET `profile_picture` = '".$filename."' WHERE `user_id` = ".$id;
    	   $this->_db->setQuery($query);
		   $this->_db->query();  
        } else {
