...
class jtagobjectdirectoryModelobject extends JModelLegacy {

    /**
     * Contains object object for current model instance
     * 
     * @var TableobjectsDirectory $_object
     */
    var $_object;
   var $_galary;

    function getOrCreateobject($id = null) {
        if ($this->_object === null) {
            $this->_object = $this->getTable('objectsDirectory');
    ...
    function save($data) {
  $query = 'SELECT id FROM `#__jtmb_objects_directory` where user_id= '.$data[user_id];
  ...
  if(!empty($rows[0]->id))
{
    ...
    $q = "UPDATE `#__jtmb_objects_directory` SET `deleted_at` = 0 WHERE `id` = '$rid'";
    $this->_db->setQuery($q);
	...
	$this->_db->query(); 
     ...
     //  print_r($data);exit;
        ...
        if ($data['id']) {
            $this->_object->load($data['id']);
        }
        $this->_object->bind($data);

//        $this->_object->object_since = $this->_object->user->registerDate;
       ...
       if($data['delete_old_galary'])       
        ...
        $this->_object->hasGallery = 1;
     

        if (!$this->_object->store()) {
            JError::raiseError(500, $this->_object->getError());
            return false;
        }
    ...
    function _saveCustomFields($fields, $user_id) {
        ...
        $query = 'DELETE FROM `#__jtmb_custom_maps_objects` WHERE user_id = ' . $user_id;
        $this->_db->setQuery($query);
     ...
     if(!empty($fields))
       {
        foreach ($fields as $field) {
            ...
            $sql = "INSERT INTO #__jtmb_custom_maps_objects (user_id, field_label,field_value)
    ...
    function _uploadPicture($force_delete_old = false) {
        ...
        if ($force_delete_old) {
            JFile::delete($dir . DS . $this->_object->profile_picture);
            JFile::delete($dir . DS . 'thumb' . DS . $this->_object->profile_picture);
            JFile::delete($dir . DS . 'small' . DS . $this->_object->profile_picture);
            $this->_object->profile_picture = '';
        }
        ...
        if ($this->_object->profile_picture) {
            JFile::delete($dir . DS . $this->_object->profile_picture);
            JFile::delete($dir . DS . 'thumb' . DS . $this->_object->profile_picture);
            JFile::delete($dir . DS . 'small' . DS . $this->_object->profile_picture);
            $this->_object->profile_picture = '';
        ...
        if (strtolower($ext) == 'jpg' || strtolower($ext) == 'png' || strtolower($ext) == 'gif' || strtolower($ext) == 'jpeg') {
            if (JFile::upload($file['tmp_name'], $dest)) {
                $this->_object->profile_picture = $filename;
 ...
 function _uploadAttachment($user_id)
  ...
  //print_r($file['name']);exit;
    ...
    { 
        $query = 'SELECT user_id FROM `#__jtmb_objects_directory` where id= '.$id;
        $this->_db->setQuery($query);
        $rows =$this->_db->loadObjectList();
   
        $delquery = 'DELETE FROM `#__jtmb_custom_maps_objects` WHERE user_id = ' . $rows[0]->user_id;
  ...
  function selectgalary($uid)
{
  ...
  if(!empty($uid))
{
    $query = 'SELECT user_id FROM `#__jtmb_objects_directory` where id='.$uid;
        $this->_db->setQuery($query);
        $rows =$this->_db->loadObjectList();
...
function softDelete($id){
	$query = "UPDATE `#__jtmb_objects_directory` SET `deleted_at` = 1 WHERE `id` = '$id'";
    $this->_db->setQuery($query);
	$this->_db->query(); 
	return;
}
...
function move($direction)
  ...
  $cid = JRequest::getVar('cid');
          $sel = 'SELECT ordering FROM `#__jtmb_objects_directory` where id='.$cid[0];
        $this->_db->setQuery($sel);
        $rows =$this->_db->loadObject();
                $ord=$rows->ordering;

         $sql='SELECT id FROM `#__jtmb_objects_directory` where ordering='.($ord+($direction));
          $this->_db->setQuery($sql);

        $row =$this->_db->loadObject();
                $id=$row->id;
        
           $query = sprintf('UPDATE `#__jtmb_objects_directory` SET `ordering`= ordering + ('.$direction.') WHERE `id`= %d ',
            $cid[0]);
    
           $this->_db->setQuery($query);
           $this->_db->query();
          $nxquery = sprintf('UPDATE `#__jtmb_objects_directory` SET `ordering`= ordering - ('.$direction.') WHERE `id`=%d',$id);
...
function saveorder($cid = array(), $order)
	{
		$row =& $this->getTable('objectsDirectory');
