     ...
     <a href="#" class="back-list" onclick="self.history.back(1)"><?php echo JText:: _('BACK_TO_THE_LIST');?></a>
        <div id="bigPic">
                <?php foreach($this->images as $image ): ?>
					<img src="components/com_jtagobjectsdirectory/assets/gallery_pictures/<?php echo $image['image'];?>"   width="700" height="400" />
                <?php endforeach; ?>
        </div>
...
<?php $id =$image['image_id']; ?>
             <ul id="thumbs">
<!-- Added by Priyanka Bhorkade on 28th Dec 2012. Functionality to delete indivisual gallary images   -->
             <li class='active' rel='1'><img src="components/com_jtagobjectsdirectory/assets/gallery_pictures/<?php echo $image['image'];?>"  /><?php $user_id = JRequest::getInt(mid); $user =& JFactory::getUser(); if($image['image'] != NULL){ if($user_id == $user->id){ ?> <input type="checkbox" name = " <?php echo "jform[$id]";?>" onClick = "<?php echo $image['image_id'] ?>" /> <?php }} ?></li>
