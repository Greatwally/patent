...
function alphaclick(alpha) {
  ...
  var url = 'index.php?option=com_jtagobjectsdirectory&format=raw&alphabet='+alpha;
  if (MooTools.version < '1.3')
  {
    var result = new Ajax(url, { method: 'get', onRequest: function() { }, onSuccess: function(data) {  $('jtag-object-list').innerHTML = data; $('jtag-md-overall-partial').setStyle('display', 'none'); }});
    result.request();
  }
  else
  {
    var result = new Request.HTML({ url: url,  method: 'get',   update: 'jtag-object-list',  onRequest: function() {} });
    result.send();
  }
