...
class jtagmemberdirectoryModelmemberDetails extends JModelLegacy
{
  function getData($id)
  {
  ...
  function _buildQuery($id)
  {
    ...
    $sql = 'SELECT id, user_id, profile_picture , first_name, last_name,Email, display_last_name,
      member_profile, country, city, state,  phone_no,facebook_page, twitter_page, member_profile
      FROM `#__jtmb_members_directory` c WHERE id = '.$id.' LIMIT 1';
 ...
 function retrive_custom($id=null)
{
   ...
   $row=$this->_db->loadObjectList();
   $uid=$row[0]->user_id;
   $query = 'SELECT distinct c.field_name, c.field_label, d.field_value, c.display_nr_cf FROM #__jtmb_custom_fields c LEFT JOIN #__jtmb_custom_maps_members  d ON d.user_id ='.$uid.' AND c.field_label=d.field_label ';
