...
class JtagMemberDirectoryModelCategories extends JModelLegacy
{
	...
	function store($data)
	{
		$row =& $this->getTable('jtagmemberdirectorycategories');
		...
		//clear cache
		$cache = & JFactory::getCache('com_jtagmembersdirectory');
		$cache->clean ('com_jtagmembersdirectory');
    ...
    /*    function edit($data)
	{
		...
		//if(is_numeric($id))
			/*
				$query =  "  SELECT m.*,s.cat_id as added FROM #__jtmb_categories m "
                         ."  LEFT JOIN #__jtmb_member_categories s  ON m.trash = 0 AND m.published = 1  AND  m.id = s.cat_id AND s.item_id = ".$id." "
      ...
      /*  function editCat() {
		...
		//$mainframe->redirect('index.php?option=com_jtagmembersdirectory&c=categories');
...
//changed by sarika
	...
	function getTotal() {
		...
		$mainframe->redirect('index.php?option=com_jtagmembersdirectory&c=categories', $msg);
	}
    ...
    function restore() {
		...
		$cache = & JFactory::getCache('com_jtagmembersdirectory');
		$cache->clean();
		$mainframe->redirect('index.php?option=com_jtagmembersdirectory&c=categories', JText::_(JTAG_CATEGORY_CALENDER_RESTORED));
	...
	function trash() {
		...
		$cache = & JFactory::getCache('com_jtagmembersdirectory');
		$cache->clean();
		$mainframe->redirect('index.php?option=com_jtagmembersdirectory&c=categories', JText::_('JTAG_CATEGORY_MOVED_TO_TRASH'));
	}  
     /* function editcat() {
		...
		//$cache = & JFactory::getCache('com_jtagmembersdirectory');
		//$cache->clean();
		$mainframe->redirect('index.php?option=com_jtagmembersdirectory&c=category');
	}  */
	function remove() {
		...
		for ($i = 0; $i < sizeof($cid); $i++) {
			...
			$query = "SELECT COUNT(*) FROM #__jtmb_member_categories WHERE cat_id={$cid[$i]}";
			$db->setQuery($query);
			$num = $db->loadResult();
		...
		$mainframe->redirect('index.php?option=com_jtagmembersdirectory&c=categories', JText::_(JTAG_CATEGORY_DELETE_MESSAGE));
	}
}
